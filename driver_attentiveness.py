...
import numpy as np
import imutils
from geo import getLongitude, getLatitude
from geocoding import findAddress
...
def calculateEAR(eye):
    ...
    EAR = (numeratorOne + numeratorTwo) / (
        2.0 * denominator
    )  # C is multiplied by 2 since we have 2 sets of vertical displacements and only 1 horizontal
    return EAR


FRAMES_BELOW_EAR_MAX = (
    30  # Number of frames where EAR is below the min and alert should sound
)

EAR_MIN = 0.24  # EAR min which indicates that EAR is below the minimum for an open eye
numOfFrames = 0  # Number of consecutive frames where EAR below min. If numOfFrames is more than FRAMES_BELOW_EAR_MAX, sound alert.results.street_address
numOfFrames2 = 0
alert = False
...
time.sleep(0.5)  # Pause for 1 second to give buffer for webcam
        ...
        if EAR < EAR_MIN:
            ...
            if numOfFrames >= FRAMES_BELOW_EAR_MAX:
                ...
                if alert == False:
                    alert = True
                led.on()
                # Do whatever you want here Moustafa
                if sent:
                    sent = 0
                    sendSms(
                        phone,
